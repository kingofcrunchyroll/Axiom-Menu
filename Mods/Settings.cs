...
namespace Seralyth.Mods
{
    public static class Settings
    {
        ...
        public static void NavigatePlayer(NetPlayer player)
        {
            ...
            {
                buttons.Add(new ButtonInfo { buttonText = "Ban Player", overlapText = $"Ban {targetName} from using Axiom", method = () => { PromptText("Reason", () => CoroutineManager.instance.StartCoroutine(BlacklistManager.SubmitBan(localUserId, player.UserId, RoleManager.GetRoleTier(localUserId).ToString(), keyboardInput, onComplete: (success, error) => { if (success) NotificationManager.SendNotification($"<color=green>Banned {targetName} successfully.</color>", 5000); else NotificationManager.SendNotification($"<color=red>Ban failed:</color> {error}", 8000); })), () => NavigatePlayer(player), "Ban", "Cancel"); SpawnKeyboard(); },isTogglable = false, toolTip = $"Bans {targetName} from Axiom" });
            }
